...
function loadAudit(){
  var url='/api/v1/audit?limit='+auditPerPage+'&offset='+auditPage*auditPerPage;
  var actF=document.getElementById('a-action').value;
  var provF=document.getElementById('a-prov').value;
  var searchF=document.getElementById('a-search').value;
  ...
  fetch(url).then(function(r){return r.json()}).then(function(data){
    ...
    if(data.providers){var sel=document.getElementById('a-prov'),cv=sel.value;
      while(sel.options.length>1)sel.removeChild(sel.lastChild);
      data.providers.forEach(function(p){var o=document.createElement('option');o.value=p;o.textContent=p;sel.appendChild(o)});
      sel.value=cv;}
...
function renderAuditTable(entries,total){
  var tbody=document.getElementById('audit-tbody');tbody.replaceChildren();
  if(!entries.length){var tr=document.createElement('tr');var td=document.createElement('td');
    td.colSpan=9;td.className='empty';td.textContent='No audit entries';
    tr.appendChild(td);tbody.appendChild(tr);}
  else{for(var i=0;i<entries.length;i++){(function(e){
    var tr=document.createElement('tr');
    var td0=document.createElement('td');td0.className='col-time';td0.textContent=fmtTime(e.timestamp);
    var td1=document.createElement('td');td1.className='col-time';td1.textContent=String(e.request_id||'');
    var td2=document.createElement('td');td2.textContent=e.provider||'';
    var td3=document.createElement('td');td3.textContent=e.model||'';
    var td4=document.createElement('td');td4.className='col-loc';td4.textContent=e.endpoint||'';td4.title=e.endpoint||'';
    var td5=document.createElement('td');
    var actionColors={pass:'info',alert:'warning',block:'critical'};
    var badge=document.createElement('span');
    badge.className='badge '+(actionColors[e.action]||'info');
    var dot=document.createElement('span');dot.className='badge-dot';badge.appendChild(dot);
    var atxt=document.createElement('span');atxt.textContent=e.action||'';badge.appendChild(atxt);
    td5.appendChild(badge);
    var td6=document.createElement('td');td6.className='col-time';
    td6.textContent=String(e.finding_count!=null?e.finding_count:(e.findings||[]).length);
    var td7=document.createElement('td');td7.className='col-time';
    td7.textContent=e.scan_duration_ms!=null?Number(e.scan_duration_ms).toFixed(1):'';
    var td8=document.createElement('td');td8.className='col-time';
    var sz=e.request_size_bytes||0;
...
function showAuditDetail(e){
  ...
  var grid=document.getElementById('audit-detail-grid');grid.replaceChildren();
  var sz=e.request_size_bytes||0;
  var sizeStr=sz>1048576?(sz/1048576).toFixed(1)+' MB':sz>1024?(sz/1024).toFixed(1)+' KB':sz+' B';
  ...
  var fdiv=document.getElementById('audit-detail-findings');fdiv.replaceChildren();
  var findings=e.findings||[];
  if(findings.length){
    var fh=document.createElement('div');fh.className='sh';fh.style.marginTop='16px';
    var fhh=document.createElement('h2');fhh.textContent='Findings ('+findings.length+')';
    fh.appendChild(fhh);fdiv.appendChild(fh);
    for(var i=0;i<findings.length;i++){(function(f){
      var card=document.createElement('div');card.className='rule-card';
      var head=document.createElement('div');head.className='rule-head';
      var nm=document.createElement('span');nm.className='rule-name';nm.textContent=f.type||'';
      head.appendChild(nm);head.appendChild(makeBadge(f.severity||'info'));
      if(f.action_taken){var tag=document.createElement('span');tag.className='action-tag';
...
function exportAudit(fmt){
  var url='/api/v1/audit/export?format='+fmt;
  var actF=document.getElementById('a-action').value;
  var provF=document.getElementById('a-prov').value;
  var searchF=document.getElementById('a-search').value;
  if(actF)url+='&action='+encodeURIComponent(actF);
