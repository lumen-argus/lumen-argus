...
class AnalyticsStore:
    ...
    def record_mcp_tool_seen(self, tool_name: str, description: str = "", input_schema: str = "") -> None:
        return self.mcp_tool_tracking.record_tool_seen(tool_name, description=description, input_schema=input_schema)

    def get_mcp_detected_tools(self) -> list[dict[str, Any]]:
        return self.mcp_tool_tracking.get_detected_tools()
    ...
    def record_mcp_tool_call(
        ...
        return self.mcp_tool_tracking.record_tool_call(
            tool_name, session_id=session_id, status=status, finding_count=finding_count, source=source
        )

    def get_mcp_tool_calls(self, session_id: str | None = None, limit: int = 100) -> list[dict[str, Any]]:
        return self.mcp_tool_tracking.get_tool_calls(session_id=session_id, limit=limit)

    def cleanup_mcp_tool_calls(self, retention_days: int = 30) -> int:
        return self.mcp_tool_tracking.cleanup_tool_calls(retention_days=retention_days)

    # --- MCP baselines facade ---

    def get_mcp_tool_baseline(self, tool_name: str) -> dict[str, Any] | None:
        return self.mcp_tool_tracking.get_baseline(tool_name)

    def record_mcp_tool_baseline(
        self, tool_name: str, definition_hash: str, description: str, param_names: list[str]
    ) -> None:
        return self.mcp_tool_tracking.record_baseline(tool_name, definition_hash, description, param_names)

    def update_mcp_tool_baseline_seen(self, tool_name: str) -> None:
        return self.mcp_tool_tracking.update_baseline_seen(tool_name)

    def increment_mcp_tool_drift_count(self, tool_name: str) -> None:
        return self.mcp_tool_tracking.increment_drift_count(tool_name)
