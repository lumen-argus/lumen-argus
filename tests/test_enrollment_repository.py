...
def _register(store, agent_id, machine_id, *, seat_cap=None):
    store.enrollment.register(
        agent_id=agent_id,
        machine_id=machine_id,
        hostname="host-%s" % machine_id,
        os="darwin",
        arch="arm64",
        agent_version="0.1.0",
...
class TestRegisterSeatCap(StoreTestCase):
    ...
    def test_seat_cap_none_allows_unlimited(self):
        for i in range(10):
            _register(self.store, "agent_%d" % i, "machine_%d" % i, seat_cap=None)
        self.assertEqual(self.store.enrollment.count_agents(), 10)

    def test_seat_cap_refuses_over_limit(self):
        _register(self.store, "agent_1", "machine_1", seat_cap=2)
        _register(self.store, "agent_2", "machine_2", seat_cap=2)

        with self.assertRaises(SeatCapExceeded) as ctx:
            _register(self.store, "agent_3", "machine_3", seat_cap=2)
    ...
    def test_seat_cap_allows_reregistration_of_same_machine(self):
        _register(self.store, "agent_1", "machine_1", seat_cap=1)
        for i in range(2, 5):
            _register(self.store, "agent_%d" % i, "machine_1", seat_cap=1)

        self.assertEqual(self.store.enrollment.count_agents(), 1)

    def test_seat_cap_zero_refuses_new_but_allows_reregistration(self):
        _register(self.store, "agent_1", "machine_1", seat_cap=None)

        # cap=0 → no new seats, but existing machine can still re-enroll.
        _register(self.store, "agent_2", "machine_1", seat_cap=0)
        self.assertEqual(self.store.enrollment.count_agents(), 1)

        with self.assertRaises(SeatCapExceeded) as ctx:
            _register(self.store, "agent_3", "machine_2", seat_cap=0)
    ...
    def test_seat_cap_exceeded_carries_current_and_cap(self):
        _register(self.store, "agent_1", "machine_1", seat_cap=3)
        _register(self.store, "agent_2", "machine_2", seat_cap=3)
        _register(self.store, "agent_3", "machine_3", seat_cap=3)

        with self.assertRaises(SeatCapExceeded) as ctx:
            _register(self.store, "agent_4", "machine_4", seat_cap=3)
