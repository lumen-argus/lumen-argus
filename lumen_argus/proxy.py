...
class ArgusProxyHandler(http.server.BaseHTTPRequestHandler):
    ...
    def _do_forward(self, request_id, server, span=None):
        ...
        try:
            ...
            # MCP-aware scanning — detect tools/call and tools/list in HTTP MCP traffic
            _mcp_info = None
            _mcp_method = None
            ...
            # MCP tools/list response — capture tool descriptions for poisoning detection
                    ...
                    if _store:
                        for t in tools_meta:
                            try:
                                ...
                                pass
                        log.debug(
                            "#%d MCP tools/list: captured %d tool descriptions",
                            request_id,
                            len(tools_meta),
                        )
